        ...
        case 'admin':
            return [
                ...
                },
                {
                    title: 'MASTER DATA',
                    items: [
                        { label: 'Kategori', href: '/admin/categories', icon: Icons.category },
                        { label: 'Supplier', href: '/admin/suppliers', icon: Icons.supplier },
                        { label: 'Obat', href: '/admin/medicines', icon: Icons.medicine },
                ...
                },
                {
                    title: 'TRANSAKSI',
                    items: [
                        { label: 'Semua Pesanan', href: '/admin/orders', icon: Icons.orders },
                    ],
                },
                {
                    title: 'SISTEM',
                    items: [
                        ...
                        { label: 'Error Logs', href: '/admin/error-logs', icon: Icons.error },
                    ],
                },
            ];
        case 'apoteker':
            return [
                {
                    title: 'UTAMA',
                    items: [
                        { label: 'Dashboard', href: '/pharmacist/dashboard', icon: Icons.dashboard },
                    ],
                },
                {
                    title: 'INVENTORI',
                    items: [
                        { label: 'Stok & Batch', href: '/pharmacist/batches', icon: Icons.batch },
                        { label: 'Riwayat Stok', href: '/pharmacist/movements', icon: Icons.history },
                    ],
                },
                {
                    title: 'PESANAN',
            ...
            return [
                {
                    title: 'UTAMA',
                    items: [
                        { label: 'Dashboard', href: '/cashier/dashboard', icon: Icons.dashboard },
                    ],
                },
                {
                    title: 'TRANSAKSI',
            ...
            return [
                {
                    title: 'UTAMA',
                    items: [
                        { label: 'Dashboard', href: '/customer/dashboard', icon: Icons.dashboard },
                    ],
                },
                {
                    title: 'BELANJA',
                    items: [
                        { label: 'Katalog Obat', href: '/customer/catalog', icon: Icons.catalog },
                        { label: 'Keranjang', href: '/customer/cart', icon: Icons.cart },
                    ],
                },
                {
                    title: 'PESANAN',
                    items: [
                        { label: 'Riwayat Pesanan', href: '/customer/orders', icon: Icons.orders },
                    ],
                },
            ];
        default:
...
export default function AppLayout({ title, children }: Props) {
        ...
        <>
            ...
            <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-5">
                {navSections.map((section) => (
                    <div key={section.title}>
                        <div className="px-3 mb-2 text-[10px] font-bold text-slate-400 tracking-widest uppercase">
                            {section.title}
                        </div>
                            ...
                            {section.items.map((item) => {
                                ...
                                return (
                                    <Link
                                        ...
                                        className={`group flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 ease-out hover:translate-x-0.5 active:scale-[0.98] ${
                                            active
                                                ? 'bg-gradient-to-r from-cyan-500/20 to-blue-500/10 text-cyan-400 border-l-[3px] border-cyan-400 -ml-px'
                                                : 'text-slate-300 hover:bg-slate-700/50 hover:text-white'
                                        }`}
                                    >
                                        <span className={`transition-transform duration-200 group-hover:scale-110 ${active ? 'text-cyan-400' : 'text-slate-400'}`}>{item.icon}</span>
                                        <span className="flex min-w-0 flex-1 items-center justify-between gap-2">
                                            <span className="truncate">{item.label}</span>
                                            {badgeCount > 0 && (
                                                <span
                                                    className={`notification-badge inline-flex min-w-5 shrink-0 items-center justify-center rounded-full px-1.5 py-0.5 text-[11px] font-bold leading-none ${
                                                        active
            ...
            <div className="p-3 border-t border-slate-700/50 shrink-0">
                <Link
                    ...
                    method="post"
                    as="button"
                    className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-slate-300 hover:bg-red-500/10 hover:text-red-400 w-full transition-all duration-200 active:scale-[0.98]"
                >
                    {Icons.logout}
                    Logout
                </Link>
    ...
    return (
        <div className="min-h-screen bg-slate-50">
            ...
            {toast && (
                <div className="fixed right-4 top-4 z-[80] w-[calc(100%-2rem)] sm:right-6 sm:top-6 sm:w-96">
                    <div
                        role="status"
                        aria-live="polite"
            ...
            {/* Mobile overlay */}
            {sidebarOpen && (
                <div className="fade-enter fixed inset-0 z-40 bg-black/50 lg:hidden" onClick={() => setSidebarOpen(false)} />
            )}

            {/* Sidebar - mobile (drawer) */}
            <aside
                className={`fixed top-0 left-0 z-50 h-full w-64 bg-slate-800 flex flex-col transform transition-transform duration-200 ease-in-out lg:hidden ${
            ...
            <aside className="hidden lg:flex lg:flex-col lg:fixed lg:inset-y-0 lg:w-64 bg-slate-800">
                <SidebarContent />
            </aside>
            ...
            <div className="lg:pl-64 flex flex-col min-h-screen">
                {/* Top bar */}
                <header className="sticky top-0 z-30 bg-white border-b border-gray-200 h-16 flex items-center justify-between px-4 sm:px-6 lg:px-8 shadow-sm">
                    <div className="flex items-center gap-4">
                        <button
                            onClick={() => setSidebarOpen(true)}
                            className="lg:hidden text-gray-500 hover:text-gray-700 transition active:scale-95"
                        >
                            {Icons.menu}
                        </button>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="relative">
                            <button
                                ...
                                className="relative text-gray-400 hover:text-gray-600 transition p-1.5 rounded-full hover:bg-gray-100 active:scale-95"
                            >
                                {Icons.bell}
                                {unreadNotificationCount > 0 && (
                                    <span className="notification-badge absolute -right-1 -top-1 inline-flex min-w-5 items-center justify-center rounded-full bg-red-500 px-1.5 py-0.5 text-[10px] font-bold leading-none text-white ring-2 ring-white">
                                        {unreadNotificationCount > 99 ? '99+' : unreadNotificationCount}
                                    </span>
                                )}
                            </button>
                            {showNotifications && (
                                <div className="popover-enter absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-lg py-1 z-50 ring-1 ring-black/5 border">
                                    ...
                                    <div className="max-h-64 overflow-y-auto">
                                        {notificationItems.length > 0 ? (
                                            notificationItems.map((notification: any) => {
                                                const content = (
                                                    <div className="flex gap-3 px-4 py-3 text-sm text-gray-600 transition hover:bg-gray-50">
                                                        <span className={`mt-1 h-2.5 w-2.5 shrink-0 rounded-full ${getNotificationAccent(notification.severity)}`} />
                                                        <span className="min-w-0">
                                                            <span className="block font-semibold text-gray-800">{notification.title}</span>
                                                            <span className="mt-0.5 line-clamp-2 block text-xs text-gray-500">{notification.message}</span>
                                                            <span className="mt-1 block text-[11px] text-gray-400">{notification.created_at}</span>
                                                        </span>
                                                    </div>
                                                );
                                            ...
                                            <div className="px-4 py-3 text-sm text-gray-600">
                                                <p className="font-semibold text-gray-800">Sistem aktif</p>
                                                <p className="mt-0.5 text-xs text-gray-400">Belum ada notifikasi baru.</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
                        ...
                        {auth?.user && (
                            <div className="flex items-center gap-3">
                                ...
                                <div className="hidden sm:block text-right">
                                    <div className="text-sm font-semibold text-slate-800 leading-tight">{auth.user.name}</div>
                                    <div className="text-[11px] text-slate-400 leading-tight">{roleLabel[role] || role}</div>
                                </div>
                            </div>
                        )}
                    </div>
                ...
                <div className="px-4 sm:px-6 lg:px-8 py-3 bg-white border-b border-gray-100">
                    <nav className="flex items-center gap-1 text-sm">
                        <Link href={navSections[0]?.items[0]?.href || '/'} className="text-gray-400 hover:text-cyan-600 transition">
                            {Icons.home}
                        </Link>
                        {breadcrumbs.length === 0 ? (
                            ...
                            breadcrumbs.map((crumb, i) => (
                                <span key={i} className="flex items-center gap-1">
                                    ...
                                    {crumb.href ? (
                                        <Link href={crumb.href} className="text-gray-400 hover:text-cyan-600 transition">
                                            {crumb.label}
                                        </Link>
                                    ) : (
