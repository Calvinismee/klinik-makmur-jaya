...
export default function AdminDashboard({ stats, topMedicines, expiringBatches, allMedicinesStock, chart }: any) {
    ...
    const normalizeSalesData = (items: any[] = [], labels: string[] = []): SalesChartDatum[] => items.map((item: any, idx: number) => {
    ...
    const periodOptions: { key: ChartPeriod; label: string; description: string }[] = [
        { key: 'daily', label: 'Harian', description: 'Ringkasan penjualan per hari' },
        { key: 'weekly', label: 'Mingguan', description: 'Ringkasan penjualan per minggu' },
        { key: 'monthly', label: 'Bulanan', description: 'Ringkasan penjualan per bulan' },
    ];
    const selectedPeriodMeta = periodOptions.find((period) => period.key === selectedPeriod) || periodOptions[0];
    const periodData = chart?.periods?.[selectedPeriod] || (selectedPeriod === 'daily' ? chart?.data : []);
    const salesChartData = normalizeSalesData(periodData || [], chart?.labels || []);
    ...
    const totalPeriodSales = salesChartData.reduce((sum, item) => sum + item.total, 0);
    const totalPeriodTransactions = salesChartData.reduce((sum, item) => sum + item.transactions, 0);
    const averageTransaction = totalPeriodTransactions > 0 ? totalPeriodSales / totalPeriodTransactions : 0;
    const currentPeriodData = salesChartData[salesChartData.length - 1];
    const bestSalesPeriod = salesChartData.reduce<SalesChartDatum | null>((best, item) => {
    ...
    const maxStock = Math.max(...stockChartData.map((item) => Math.max(item.stock, item.minimumStock)), 1);
    const dashboardCharts = (
        <div className="grid grid-cols-1 gap-8 mb-8 xl:grid-cols-[minmax(0,2fr)_minmax(340px,1fr)]">
            <div className="bg-white rounded-lg shadow-sm p-6">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                        <h2 className="text-lg font-bold">Grafik Penjualan & Pendapatan</h2>
                        <p className="mt-1 text-sm text-gray-500">{selectedPeriodMeta.description}. Arahkan kursor ke bar untuk detail.</p>
                    </div>
                ...
                <div className="mt-5 grid grid-cols-1 border-y border-gray-200 sm:grid-cols-2 lg:grid-cols-4 lg:divide-x lg:divide-gray-200">
                    <div className="py-4 lg:pr-5">
                        <div className="text-xs font-semibold uppercase text-gray-500">Pendapatan {selectedPeriodMeta.label}</div>
                        <div className="mt-1 text-xl font-bold text-gray-900">{formatCurrency(totalPeriodSales)}</div>
                        <div className="mt-1 text-sm text-gray-500">{totalPeriodTransactions} transaksi</div>
                    </div>
                    <div className="border-t border-gray-200 py-4 sm:border-t-0 sm:pl-5 lg:px-5">
                        <div className="text-xs font-semibold uppercase text-gray-500">Periode Terbaik</div>
                        <div className="mt-1 text-xl font-bold text-gray-900">
                            {bestSalesPeriod && bestSalesPeriod.total > 0 ? bestSalesPeriod.label : '-'}
                        </div>
                        <div className="mt-1 text-sm text-gray-500">
                            {bestSalesPeriod && bestSalesPeriod.total > 0 ? formatCurrency(bestSalesPeriod.total) : 'Belum ada penjualan'}
                        </div>
                    </div>
                    <div className="border-t border-gray-200 py-4 sm:pr-5 lg:border-t-0 lg:px-5">
                        <div className="text-xs font-semibold uppercase text-gray-500">Periode Terakhir</div>
                        <div className="mt-1 text-xl font-bold text-gray-900">{formatCurrency(currentPeriodData?.total || 0)}</div>
                        <div className="mt-1 text-sm text-gray-500">{currentPeriodData?.transactions || 0} transaksi</div>
                    </div>
                    <div className="border-t border-gray-200 py-4 sm:border-t-0 sm:pl-5 lg:pl-5">
                        <div className="text-xs font-semibold uppercase text-gray-500">Rata-rata Transaksi</div>
                        <div className="mt-1 text-xl font-bold text-gray-900">{formatCurrency(averageTransaction)}</div>
                        <div className="mt-1 text-sm text-gray-500">Nilai rata-rata order</div>
                    </div>
                </div>
                ...
                <div className="mt-5 overflow-x-auto">
                    <div className="min-w-[760px]">
                        <div className="relative h-80 border-b border-l border-gray-200 pl-20">
                            <div className="absolute inset-y-0 left-0 right-0 flex flex-col justify-between pb-14 pt-8">
                                {[maxSales, maxSales / 2, 0].map((value, idx) => (
                                    <div key={idx} className="flex items-center gap-3">
                                        <span className="w-16 shrink-0 whitespace-nowrap text-right text-[11px] font-medium text-gray-500">
                            ...
                            <div className="relative z-10 flex h-full items-end gap-4 px-4 pb-14 pt-8">
                                {salesChartData.map((day, idx) => {
                                    ...
                                    return (
                                        <div key={`${day.date}-${idx}`} className="group relative flex min-w-[78px] flex-1 flex-col items-center justify-end">
                                            <div className="pointer-events-none absolute left-1/2 top-4 z-20 hidden w-64 -translate-x-1/2 rounded-md bg-gray-900 p-3 text-xs text-white shadow-lg group-hover:block">
                                            ...
                                            <div className="mt-2 max-w-[92px] text-center text-xs font-medium text-gray-600">{day.label}</div>
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
                <div className="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-gray-500">Obat dengan stok terendah dibanding threshold.</p>
                    </div>
                ...
                <div className="mt-5 grid grid-cols-3 gap-3 border-y border-gray-200 py-4 text-center">
                    <div>
                        <div className="text-lg font-bold text-gray-900">{stockSummary.totalMedicines || 0}</div>
                        <div className="mt-1 text-[11px] font-semibold uppercase text-gray-500">Jenis Obat</div>
                    </div>
                    <div>
                        <div className="text-lg font-bold text-gray-900">{stockSummary.totalUnits || 0}</div>
                        <div className="mt-1 text-[11px] font-semibold uppercase text-gray-500">Total Unit</div>
                    </div>
                    <div>
                        <div className="text-lg font-bold text-gray-900">{stockSummary.empty || 0}</div>
                        <div className="mt-1 text-[11px] font-semibold uppercase text-gray-500">Stok 0</div>
                    </div>
                </div>

                <div className="mt-5 space-y-4">
                    {stockChartData.map((item) => {
                        const stockPercent = Math.max(2, Math.min(100, (item.stock / maxStock) * 100));
                        const thresholdPercent = Math.min(100, (item.minimumStock / maxStock) * 100);
                        ...
                        return (
                            <div key={item.id} className="group">
                                <div className="mb-1 flex items-center justify-between gap-3">
                                    ...
                                    <div className={`text-sm font-bold ${isCritical ? 'text-red-600' : 'text-emerald-600'}`}>
                                        {item.stock}
                                    </div>
                                </div>
                                ...
                                <div className="mt-1 flex justify-between text-[11px] text-gray-400">
                                    ...
                                    <span>{isCritical ? 'Di bawah minimum' : 'Aman'}</span>
                                </div>
                            </div>
                        );
                    })}
