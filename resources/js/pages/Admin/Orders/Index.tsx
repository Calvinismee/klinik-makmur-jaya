...
export default function AdminOrdersIndex({
    ...
    const isOfflineOrder = (order: any) =>
        String(order.order_number || '').startsWith('POS-');
    const formatPurchaseType = (order: any) =>
        isOfflineOrder(order) ? 'Offline' : 'Online';
    const getPurchaseTypeColor = (order: any) =>
        isOfflineOrder(order)
            ? 'bg-slate-100 text-slate-800'
            : 'bg-cyan-100 text-cyan-800';
    const formatCustomerName = (order: any) =>
        isOfflineOrder(order) ? 'Pelanggan Offline' : order.user?.name || '-';
                ...
                <div className="mb-6 flex items-center justify-between border-b pb-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                        <button
                            ...
                            onClick={() =>
                                router.post(
                                    '/admin/orders/export/excel/background',
                                    {},
                                    { preserveScroll: true },
                                )
                            }
                        ...
                        <button
                            ...
                            onClick={() =>
                                router.post(
                                    '/admin/orders/export/pdf/background',
                                    {},
                                    { preserveScroll: true },
                                )
                            }
                        ...
                        {!showReportProgress && jobs.length > 0 && (
                            <button
                                type="button"
                                onClick={() =>
                                    setReportProgressVisibility(true)
                                }
                        ...
                        <div className="flex items-center justify-between gap-3">
                            <div>
                                <h2 className="text-sm font-bold text-slate-900">
                                    Progress Laporan Background
                                </h2>
                                <p className="mt-0.5 text-xs text-slate-500">
                                    Progress diperbarui otomatis selama queue
                                    worker berjalan.
                                </p>
                            </div>
                            <div className="flex items-center gap-2">
                                ...
                                <button
                                    type="button"
                                    onClick={() =>
                                        setReportProgressVisibility(false)
                                    }
                                    className="btn-secondary px-3 py-1 text-xs"
                                >
                                    Sembunyikan
                            ...
                            {jobs.map((job) => (
                                ...
                                >
                                    <div className="flex flex-wrap items-center justify-between gap-2">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span
                                                className={`rounded-full px-2 py-1 text-xs font-bold ${getJobStatusColor(job.status)}`}
                                            >
                            ...
                            {orders.map((order) => (
                                <tr key={order.id}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <span
                                            className={`inline-flex rounded-full px-2 text-xs leading-5 font-semibold ${getPurchaseTypeColor(order)}`}
                                        >
